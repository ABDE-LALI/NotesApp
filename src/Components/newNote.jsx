...
export default function NewNote(props) {
    ...
    const [notetext, setnotetext] = useState("Note default content");
    ...
    const save = async () => {
        if (notetitle && notetext) {
            let resp;
            try {
                resp = await axios.post(`https://notes.devlop.tech/api/notes`, { "title": notetitle, "content": notetext, 'shared_with': choosedusers.map(user => user.id) }, {
                    headers:
    ...
    return <>
        <div className="newnote" onClick={() => { shusers && hideusers(false); console.log("hello") }
            ...
            <textarea className="typefield_body" type="text" value={notetext} onChange={(e) => setnotetext(e.target.value)} />
            <span>
